...
public:
        typedef std::num_get<_CharT, _Iter> _MyBaseT;
        ...
        explicit extended_num_get(size_t _Refs = 0)
                : facet(_Refs),
                  _Base(std::use_facet<_MyBaseT>(std::locale())) {}
        extended_num_get(_MyBaseT & _NumGet, size_t _Refs = 0)
                : facet(_Refs),
                  _Base(_NumGet) {}
        ...
        _Iter get(_Iter _In, _Iter _End, std::ios_base & _B,
                ...
                { return _Base.get(_In, _End, _B, _Err, _V); }
        _Iter get(_Iter _In, _Iter _End, std::ios_base & _B,
...
locale::id extended_num_get<_CharT, _Iter>::id;
...
public:
        typedef std::num_put<_CharT, _Iter> _MyBaseT;
        ...
        explicit extended_num_put(size_t _Refs = 0)
                : facet(_Refs),
                  _Base(std::use_facet<_MyBaseT>(std::locale())) {}
        extended_num_put(_MyBaseT & _NumPut, size_t _Refs = 0)
                : facet(_Refs),
                  _Base(_NumPut) {}
...
template <class _CharT, class _Iter>
locale::id extended_num_put<_CharT, _Iter>::id;
...
std::basic_string<_CharType> _DecNumber::_DoPut(const std::ios_base & _F,
        ...
        typedef std::moneypunct<_CharType> _MPunctT;
        _CharType _DecPt =
                std::use_facet<_MPunctT>(_F.getloc()).decimal_point();
        if (_DecPt == 0)
...
static const decNumberUnit _PowersOfTen[] = { 1, 10, 100, 1000,
...
_CharType _DecodeDigit(const unsigned char _Dig,
        ...
        decNumberUnit _Pow = _PowersOfTen[_Dig - 1] * 8;
...
_Iter _DecNumber::_GetMantissa(_Iter _Beg,
        ...
        bool _FoundNonZeroDigit = false;
        for (; _Beg != _End; ++_Beg)
                {
                if (_CType.is(std::ctype_base::digit, *_Beg))
                        {
                        _Acc.push_back(*_Beg);
                        if (!_FoundNonZeroDigit && *_Beg != _Zero)
                                { _FoundNonZeroDigit = true; }
                        }
                else if (*_Beg == _DecPt)
                        { _FoundDecPt = true; }
                else
        ...
        digits = _Acc.length();
        exponent = _FoundNonZeroDigit ? _DecPtPos - digits : 0;
